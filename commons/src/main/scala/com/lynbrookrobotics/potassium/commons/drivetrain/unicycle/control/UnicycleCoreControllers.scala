...
import squants.{Angle, Length, Percent, Quantity, Time}
...
trait UnicycleCoreControllers {
  ...
  def openLoopToDriveSignal(openLoopInput: OpenLoopInput): DriveSignal

  def childOpenLoop(unicycle: Stream[UnicycleSignal]): Stream[OpenLoopInput]

  def childVelocityControl(unicycle: Stream[UnicycleSignal])(implicit hardware: DrivetrainHardware,
