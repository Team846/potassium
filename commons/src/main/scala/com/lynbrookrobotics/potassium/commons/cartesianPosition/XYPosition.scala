...
object XYPosition {
  ...
  /**
    ...
    */
  def circularTracking(angle: Stream[Angle],
                       distanceTraveled: Stream[Length]): Stream[Point] = {
    val centralAngle = angle.sliding(2).map(angleQueue =>
      angleQueue.last - angleQueue.head
    )
    val arcLength = distanceTraveled.sliding(2).map(arcQueue =>
      arcQueue.last - arcQueue.head
    )
    val radius = centralAngle.zip(arcLength).map { case (centralAngle, arcLength) =>
    ...
    centralAngle.zip(previousAngle).zip(radius).zip(arcLength).zip(angle).scanLeft(Point.origin) {
      case (prevPos, ((((centralAngle, prevAngle), radius), arcLength), currAngle)) =>
        ...
        if (centralAngle.toDegrees == 0.0) {
          Point(
            x = prevPos.x + arcLength * currAngle.cos,
            y = prevPos.y + arcLength * currAngle.sin)
        } else {
