...
trait PurePursuitControllers extends UnicycleCoreControllers with UnicycleMotionProfileControllers {
   ...
   */
  def pointDistanceControl(
    position: Stream[Point],
    target: Stream[Point],
    maxVelocity: Velocity,
    acceleration: Acceleration,
    deceleration: Acceleration
  )(implicit properties: Signal[DrivetrainProperties], hardware: DrivetrainHardware): Stream[Dimensionless] = {
  ...
  def purePursuitControllerTurn(
  ...
  ): (Stream[Dimensionless], Stream[Double]) = {
    val lookAheadPoint = position.zip(biSegmentPath).map {
      case (pose, path) =>
    ...
    val errorToTarget = headingToTarget.map(convertTrigonometricAngleToCompass).zip(turnPosition).map {
      case (target, current) =>
        target - current
    }
    ...
    val compassHeadingToLookAheadAndReversed: Stream[(Angle, Boolean)] = errorToTarget
      ...
      .zip(lookAheadPoint.map(_._2))
      .map {
        case ((angle, autoReversed), purposefullyReversed) =>
    ...
    val limitedAndWithinDeadband = limitedTurn.zip(errorToTarget).map {
      case (turn, error) =>
   ...
   */
  @tailrec
  final def getExtrapolatedLookAheadPoint(
    biSegmentPath: (Segment, Option[Segment]),
    currPosition: Point,
    lookAheadDistance: Length
  ): (Point, Boolean) = {
    val lookAheadOnCurrentSegment =
      intersectionRayCircleFurthestFromStart(biSegmentPath._1, currPosition, lookAheadDistance)
    ...
    if (lookAheadOnNextSegment.isDefined) {
      ...
      val angleRobotProjectionToEnd =
        Segment(biSegmentPath._2.get.pointClosestToOnLine(currPosition), biSegmentPath._2.get.end).angle
      ...
      val angleRobotProjectionToEnd =
        Segment(biSegmentPath._1.pointClosestToOnLine(currPosition), biSegmentPath._1.end).angle
  ...
  def followWayPointsController(
  ...
  ): (Stream[UnicycleSignal], Stream[Option[Length]]) = {
    println("in follow way points controller")
    val biSegmentPaths = wayPoints
      .sliding(2)
      .map { points =>
        Segment(
          points.head,
          points.tail.headOption.getOrElse(
            throw new IllegalArgumentException(
              "There must be at least two waypoints. Did you forget to add the initial point?"
            )
          )
        )
      }
      .sliding(2)
      .map(l => (l.head, l.tail.headOption))
    ...
    val selectedPath = position.map { currPos =>
      if (biSegmentPaths.hasNext &&
          currPath._2.exists(
            s => intersectionRayCircleFurthestFromStart(s, currPos, props.get.defaultLookAheadDistance).isDefined
          )) {
    ...
    (
      limitedAndReversedForward.zip(turnOutput).map {
        case (forward, turn) =>
