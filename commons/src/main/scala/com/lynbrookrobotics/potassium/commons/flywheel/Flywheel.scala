...
trait FlywheelProperties {
  def velocityGains
    : PIDFConfig[Frequency, GenericValue[Frequency], Frequency, GenericDerivative[Frequency], Dimensionless, Dimensionless]
}
...
abstract class Flywheel {
  ...
  object velocityControllers {
    def velocityControl(
      target: Stream[Frequency]
    )(implicit properties: Signal[Properties], hardware: Hardware): (Stream[Frequency], Stream[Dimensionless]) = {
  ...
  object velocityTasks {
    class WhileAtVelocity(vel: Stream[Frequency], tolerance: Frequency)(flywheel: Comp)(
