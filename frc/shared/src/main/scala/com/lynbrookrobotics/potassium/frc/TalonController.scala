...
import com.ctre.phoenix.motorcontrol.{ControlMode, FeedbackDevice, VelocityMeasPeriod}
...
class TalonController(deviceNumber: Int) {
  ...
  def setOutput (output: Double): Unit = talon.set(ControlMode.PercentOutput, output)
